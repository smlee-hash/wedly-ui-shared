...
import { recapKindLabel, safeRecapKind, safeRecapLines, type CommentRecap, type RecapKind } from "../unified/history-core";

type KindStyle = { tile: string; symbol: string; icon: LucideIcon };

const KIND: Record<RecapKind, KindStyle> = {
  call:     { tile: "bg-wedly-accent", symbol: "text-white",       icon: Phone },
  contract: { tile: "bg-wedly-green",  symbol: "text-white",       icon: FileCheck2 },
  document: { tile: "bg-wedly-purple", symbol: "text-white",       icon: FileText },
  schedule: { tile: "bg-wedly-navy",   symbol: "text-white",       icon: CalendarClock },
  // ★경고 타일만 남색 심볼 — 금색 위 흰 글리프는 대비 2.1 로 미달이다(디자인 시스템 상태박스 v3).
  issue:    { tile: "bg-wedly-gold",   symbol: "text-wedly-navy",  icon: AlertTriangle },
  note:     { tile: "bg-wedly-navy",   symbol: "text-white",       icon: StickyNote },
};
    ...
    <div className="ml-7 overflow-hidden rounded-xl border border-wedly-bd bg-white shadow-[0_1px_2px_rgba(10,34,68,0.05),0_6px_18px_rgba(10,34,68,0.08)]">
      <div className="flex items-start gap-2.5 px-3 pb-2 pt-3">
        ...
        <div className="min-w-0">
          <p className="text-[10.5px] font-medium leading-tight text-wedly-muted">{recapKindLabel(recap.kind)}</p>
