...
export type CollabTableProps = {
   ...
   *
   * ⚠️ 반드시 안정된 함수를 넘긴다(useCallback 등) — 검색 결과 계산이 이 함수 참조에 의존하므로,
   * 매 렌더 새로 만든 함수를 넘기면 검색 중 매 렌더마다 전 행 재필터·재정렬·재페이지가 돈다.
   * (아래 mobile.renderSubtitleLeft 는 계산 의존성에 안 들어가 이 요건이 없다 — 취급이 다르니 주의.)
   */
  ...
  /**
   * 줄 전체 색칠(조건부 서식). 그 줄의 데이터로 색 클래스(예: "bg-wedly-bg-red text-wedly-red-ink")를 돌려주면
            ...
            <div className="border-t border-wedly-bd px-3 py-2">
              {addingNew ? (
                ...
                <button
                  ...
                  className="w-full rounded-lg border border-wedly-bd-blue bg-wedly-bg-blue px-2 py-1 text-[12px] text-wedly-accent-ink hover:bg-wedly-bg-blue/70 transition-colors text-left"
                >
                  + 새 옵션
                </button>
              )}
