 ...
 *
 * ★물결표(~)는 **둘 붙은 것(취소선 `~~`)만** 지운다(2026-09-02 태산레져 실사례):
 *  홑 물결표는 우리말에서 범위 기호다 — `24~25억`·`3~6개월` 을 지우면 `2425억`·`36개월` 이 되어
 *  대표님께 매출이 100배 부풀려 나간다. 마크다운에서 홑 물결표는 장식이 아니다.
 */
export function stripMarkdownKeepUrls(v: string): string {
  return v
    .split(/(\bhttps?:\/\/\S+)/g)
    .map((조각, i) => (i % 2 === 1 ? 조각 : 조각.replace(/~~/g, "").replace(/[*_`#>|]/g, "")))
    .join("");
}
