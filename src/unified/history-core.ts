...
/** 상한 — 깨진 답·이관 기록에 개행이 수만 개 들어와도 화면이 멎지 않게(코덱스 지적). */
export const KAKAO_TEXTAREA_MAX_ROWS = 400;
 ...
 *  이 값은 첫 그리기용(개행 기준)이고, 자동 줄바꿈으로 접히는 줄은 대화상자가 그린 뒤 실제 높이(scrollHeight)로 맞춘다.
 */
export function kakaoTextareaRows(text: string, min = KAKAO_TEXTAREA_MIN_ROWS, max = KAKAO_TEXTAREA_MAX_ROWS): number {
  const s = String(text ?? "");
  let lines = 1;
  for (let i = 0; i < s.length && lines <= max; i++) if (s.charCodeAt(i) === 10) lines++;
