...
// ─── 공통 12칸 (하이브·ERP·일루아 공유) ───
// 팀장·팀원은 앱별로 다르므로 제외. 환급금여부는 공통 신규 추가.
// 진행상태: NO.56 사장님 결정 — 3앱 공통(값 공유, 어디서 바꾸든 동기화). 키가 앱·업무별로 달라
//   (경정=05경정계약진행상태, 정부지원금/일루아=05진행상태, 노무=진행상태, 무료=progressStatus) 모두 후보키로 넣어 같은 '진행상태'로 묶는다.
...
export const COMMON_BASIC_FIELD_SPECS: BasicFieldSpec[] = [
  ...
  { label: "진행상태",       keys: ["05경정계약진행상태", "05진행상태", "진행상태", "progressStatus"], type: "select" },
];
